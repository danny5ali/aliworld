...
class AccessoryScene extends Phaser.Scene {
  ...
  init(data) {
    ...
    // ensure inventory exists. if first time, give them the starters.
    ...
    // working state - clone current loadout so cancel works
  ...
  buildEquipSlots() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      this._slotObjects.push({ bg, placeholder, labelText, slotNumText, x, y });
    }
  }
  ...
  drawInventoryItems() {
    ...
    this._inventory.forEach((item, i) => {
      ...
      // placeholder visual: colored circle with letter
      const color = STAT_COLOR[item.stat] || 0x666677;
      const circle = this.add.circle(x, y - 8, 14, color, 0.7).setStrokeStyle(1, 0xffffff);
      const letter = this.add.text(x, y - 8, item.stat[0].toUpperCase(), {
        fontFamily:'monospace', fontSize:'14px', fontStyle:'bold', color:'#ffffff'
      }).setOrigin(0.5);

      const name = this.add.text(x, y + 14, item.shortName || item.name.split(' ')[0], {
      ...
      bg.on('pointerover', () => {
        ...
        this.showTooltip(item, x, y - itemSize);
      });
      bg.on('pointerout', () => {
  ...
  refreshStatPreview() {
    ...
    const fmt = (label, b, bn) => {
      const total = b + bn;
      const sign = bn > 0 ? `+${bn}` : (bn < 0 ? `${bn}` : '');
      const signColor = bn > 0 ? '[color=#44cc88]' : (bn < 0 ? '[color=#cc4444]' : '');
      return `${label} ${total}` + (sign ? `  (${sign})` : '');
    };
  ...
  refreshSlotDisplay() {
    this._slotObjects.forEach(({ bg, placeholder, labelText, slotNumText, x, y }, i) => {
      placeholder.clear();
      const item = this._equipped[i];
      if (item) {
        const color = STAT_COLOR[item.stat] || 0x666677;
        placeholder.fillStyle(color, 0.85);
        placeholder.lineStyle(1, 0xffffff, 0.9);
        placeholder.fillCircle(x, y - 6, 14);
        placeholder.strokeCircle(x, y - 6, 14);
  ...
  showTooltip(item, x, y) {
    this.hideTooltip();
    const { width } = this.scale;
    const tx = Math.min(Math.max(x, 100), width - 100);
    const ty = Math.max(y - 30, 20);

    const bonuses = [];
    if (item.bonuses) {
      Object.entries(item.bonuses).forEach(([k, v]) => {
        const sign = v > 0 ? '+' : '';
        const color = v > 0 ? '#44cc88' : '#cc4444';
        bonuses.push(`${sign}${v} ${k.toUpperCase()}`);
      });
    }

    this._tooltip = [
      this.add.rectangle(tx, ty - 6, 180, 62, 0x1a1a2a)
      ...
      this.add.text(tx, ty - 6, bonuses.join('   '), {
        fontFamily:'monospace', fontSize:'10px', color:'#aaaabb'
      }).setOrigin(0.5).setDepth(201),
      this.add.text(tx, ty + 10, item.desc || '', {
        fontFamily:'monospace', fontSize:'9px', color:'#666677',
        wordWrap:{ width:172 }, align:'center'
      }).setOrigin(0.5).setDepth(201),
    ];
  }
...
// 5 starters - small bonuses with small drawbacks. real tradeoffs.
...
// boss drops - pure positive. earned, not chosen.
const BOSS_DROPS = {
...
// minor enemy drops - random, episode-themed. 30% chance to drop.
const MINOR_DROPS = {
